  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-8">
        ...
        <div><h1 className="text-xl font-bold text-slate-800 font-heading">{t.title}</h1><p className="text-sm text-slate-500">{transactions.length} transaction{transactions.length !== 1 ? "s" : ""}</p></div>
      </div>
              ...
              <Card key={txn.id}><div className="p-5">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div><p className="text-sm font-semibold text-slate-800">{txn.type.replace(/_/g, " ").toLowerCase().replace(/\b\w/g, (l) => l.toUpperCase())}</p><p className="text-xs text-slate-400">{timeAgo(txn.createdAt)}</p></div>
                  </div>
